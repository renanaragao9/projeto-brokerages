...
import { getConstructionUpdates, getProperty } from "@/lib/api";
import {
...
export default async function EmpreendimentoDetailPage(props: PageProps<"/canopus/[id]">) {
  ...
  const constructionUpdates = await getConstructionUpdates(property.id).catch(() => []);
    ...
    <>
            ...
            {specs.length > 0 && (
              <Reveal className="mt-10 grid grid-cols-2 overflow-hidden rounded-2xl border border-brand-100 bg-white sm:grid-cols-3">
                {specs.map(({ Icon, label, value }) => (
                  <div
                    key={label}
                    className="flex items-start gap-3 border-b border-r border-brand-100 p-5 [&:nth-child(2n)]:border-r-0 sm:[&:nth-child(2n)]:border-r sm:[&:nth-child(3n)]:border-r-0"
                  >
              ...
              <Reveal className="mt-12">
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  {notices.map((notice) => (
                    <Link
                      key={notice.id}
                      href={`/canopus/noticias/${notice.slug}`}
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            <div className="rounded-2xl bg-white p-7 shadow-[0_24px_60px_-34px_rgba(0,32,65,0.7)] ring-1 ring-brand-900/8">
              ...
              <dl className="mt-7 space-y-3 border-t border-brand-100 pt-6 text-sm">
                ...
                {constructionPhaseLabel(property) && (
                  <div className="flex justify-between gap-4">
                    <dt className="text-brand-900/45">Fase da obra</dt>
                    <dd className="font-medium text-brand-800">
                      {constructionPhaseLabel(property)}
                    </dd>
                  </div>
      ...
      <section className="bg-brand-50/60 py-16 lg:py-20">
        <div className="mx-auto w-full max-w-7xl px-5 lg:px-8">
          <Reveal>
            <ConstructionUpdates propertyId={property.id} updates={constructionUpdates} />
          </Reveal>
        </div>
      </section>
