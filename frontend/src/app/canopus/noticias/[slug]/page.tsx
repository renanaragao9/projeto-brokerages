...
const NOTICEABLE_LINK: Record<string, (id: number) => string> = {
  property: (id) => `/canopus/${id}`,
};

export async function generateMetadata(props: PageProps<"/canopus/noticias/[slug]">): Promise<Metadata> {
...
export default async function NoticiaDetailPage(props: PageProps<"/canopus/noticias/[slug]">) {
  const { slug } = await props.params;
  const notice = await getNotice(slug).catch(() => null);
        ...
        <article className="mx-auto w-full max-w-3xl px-5 lg:px-8">
          ...
          {notice.content && (
            <Reveal className="notice-content text-base leading-relaxed text-brand-900/75">
              {/* Conteúdo vem do RichEditor do painel administrativo, controlado pela equipe. */}
