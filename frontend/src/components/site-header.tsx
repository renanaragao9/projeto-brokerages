...
export function SiteHeader({ config = site }: { config?: SiteConfig }) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-20 w-full max-w-7xl items-center gap-6 px-5 lg:px-8">
        <Link href={config.rootPath} className="flex items-center gap-3" onClick={() => setOpen(false)}>
          <Image
            src={config.logo}
            alt={config.name}
            width={40}
        ...
        <a
          href={whatsappLink(`Olá! Vim pelo site da ${config.name} e quero falar com um corretor.`, config)}
          target="_blank"
